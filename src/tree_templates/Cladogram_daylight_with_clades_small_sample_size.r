...
Clado_daylight <- function(tree, metadata, color_mapping, OffS_C) {
  # Start with the base ggtree plot, using the phylo object
  p <- ggtree(tree, color = "#646567", size = 0.5, branch.length = "none", layout = "daylight") +
              geom_tippoint(aes(fill = Node_color, angle = angle), color = "#000000", shape = 22, size = 1, alpha = .75
                            ) +
              geom_nodepoint(aes(fill = Node_color), color = "#000000", shape = 21, size = 1
                            ) + # Add nodes
              geom_tiplab(aes(fill = clade, angle = "angle"), align = FALSE, alpha = 0.7, color = "#000000", hjust = -0.1, size = 1.5, offset = 0.0, geom = "label"
                            ) +
  ...
  mrca_nodes <- metadata %>%
    filter(!is.na(mrca_node) & !is.na(clade)) %>% # Exclude NA
    group_by(clade) %>%
    filter(n() > 1) %>%   # Only keep clades with more than one member
    ungroup() %>%
  ...
  p <- p +
    geom_cladelab(
      ...
      align = FALSE,
      hjust = "center",
      offset = OffS_C,           # Offset for visibility
      offset.text = 0.4,      # Offset for the text
      barsize = 0.8,          # Size of line
