...
Clado_rect <- function(tree, metadata, color_mapping) {
  ...
  p <- ggtree(tree, color = "#646567", size = 1, branch.length = "none") +
              geom_tippoint(aes(fill = Node_color), color = "#000000", shape = 22, size = 2, alpha = .75
                            ) +
              geom_tiplab(aes(fill = clade), align = TRUE, alpha = 0.7, color = "#000000", size = 1.8, offset = 1.0, geom = "label"
                          ) +
