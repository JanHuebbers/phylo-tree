...
Clado_circ <- function(tree, metadata, color_mapping) {
  ...
  p <- ggtree(tree, color = "#646567", size = 0.5, branch.length = "none", layout = "circular") +
              geom_tippoint(aes(fill = Node_color, angle = angle), color = "#000000", shape = 22, size = 1, alpha = .75
                            ) +
              geom_tiplab(aes(fill = clade, angle = angle), align = TRUE, alpha = 0.7, color = "#000000", size = 1.5, offset = 0.5, geom = "label"
                          ) +
  ...
  p <- p +
    geom_cladelab(
      ...
      align = TRUE,
      hjust = "center",
      offset = 2.5,           # Offset for visibility
